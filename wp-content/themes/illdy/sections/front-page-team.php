		...
		<div class="container">
			<div class="row">
				...
				if( is_active_sidebar( 'front-page-team-sidebar' ) ):
					...
					the_widget( 'Illdy_Widget_Person', 'title='. __( 'Genilson Vianna', 'illdy' ) .'&image='. esc_url( '/layout/images/front-page/genilson-vianna-atual.JPG' ) .'&position='. __( 'Ministro de Louvor', 'illdy' ) .'&entry='. __( 'Locutor da "noites boas novas"', 'illdy' ) .'&facebook_url='. esc_url( '#' ) .'&twitter_url='. esc_url( '#' ) .'&linkedin_url='. esc_url( '#' ) .'&color=#6a4d8a', $the_widget_args );
				endif;
				?>
			</div><!--/.row-->
		</div><!--/.container-->
